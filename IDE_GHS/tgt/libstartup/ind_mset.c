...
/* heavily unrolled loop, using up to 16-byte alignment (for i960) */
...
/* [JY] I reversed the order of the writes, since we have to remember the base*/
...
/* Old way _may_ be better on 68k, with its cisc logic, but maybe not. */
...
void *memset(void *s, int c, size_t n) {
	...
	/* do this first because it will use several temporary registers */
	ch4 = ch;
	ch4 <<= 8;
	ch4 |= ch;
	ch4 = (ch4 << 16 | ch4);

	/* get rid of odd bytes at the end first, and stop using n */
...
#endif
	    /* [jonhsu] Mon Jun 29 11:38:05 PDT 1998  The usage of the simple
	       comparison between pt and s doesn't account for when s is
	       at or below zero while pt is above zero. */
#if __PTR_BIT < __INT_BIT && __PTR_BIT == __SHRT_BIT
	    stop2 = (unsigned char*)(short)((size_t)s&(~(LOOP-1)));
#else
	    stop2 = (unsigned char*)((size_t)s&(~(LOOP-1)));
#endif
/*[TPW/edlee] Thu Feb 29 00:51:24 PST 1996.
	Jeff's turbo memset() enhancement failed to compensate for overshoot
	on a pointer it was walking backwards through the memory block. Add
	one instruction after the loop to correct this. Also, change his
	termination test to better optimize the case where the start of the
	memory block is favorably aligned. 2 places. */
	    /* [jonhsu] Mon Jun 29 11:38:05 PDT 1998  The usage of the simple
	       comparison between pt and s doesn't account for when s is
	       at or below zero while pt is above zero.  */
	    while ((pt -= LOOP) != stop2) {
...
void *memset(void *s, int c, size_t n) {
	...
	/* we are now at an block boundary. */
...
#endif
	    do {
		/* loop body is 8 instructions, loop overhead is 3 instructions */
#ifdef USE_STRUCT16
		((struct quad*)pt)[0] = ch16;
		((struct quad*)pt)[1] = ch16;
