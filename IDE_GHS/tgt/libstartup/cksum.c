/*
 *  C Runtime library Checksum Verification
 *
 *  Copyright 1983-2009
...
#if defined(__ZSP)
	/* Architecture has separate text and data spaces */
#define LOAD_FROM_TEXT if (text) asm("bits %smode, 5")
#define LOAD_FROM_DATA if (text) asm("bitc %smode, 5")
#else	/* Unified memory architecture */
#define LOAD_FROM_TEXT
#define LOAD_FROM_DATA
#endif
...
figure 19-5.
*/
#define CRC_POLYNOMIAL 0x10211021
static int32_t
